...
const driverSignupStart = asyncHandler(async (req, res) => {
    try {
        ...
        const data = await AuthService.driverSignupStart({ name, phone, email, password });
...
const driverSignupDocuments = asyncHandler(async (req, res) => {
    try {
        ...
        console.error('Driver Signup Documents Error:', error);
        if (error.statusCode === 400) return sendValidationError(res, error.message);
        return sendError(res, error.statusCode || 500, error.message);
    }
});
...
const driverSignupComplete = asyncHandler(async (req, res) => {
    try {
        ...
        return sendSuccess(res, 201, 'Driver account created successfully', { token, user });
    } catch (error) {
